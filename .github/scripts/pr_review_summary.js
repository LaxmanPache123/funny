...
async function main() {
  try {
    const prs = await fetchAllPages('GET /repos/{owner}/{repo}/pulls', { owner, repo, state: 'all' });

    // Count PRs merged today
    const mergedTodayCount = prs.filter(pr => pr.merged_at && pr.merged_at.startsWith(today)).length;
